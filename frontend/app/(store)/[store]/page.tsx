...
export async function generateMetadata({ params }: { params: Promise<{ store: string }> }) {
  ...
  const description = seller.description?.slice(0, 160) ?? `Shop at ${name} on StoreHubBD`
  return {
    title: name,
    description,
    openGraph: {
...
export default async function StorePage({ params }: { params: Promise<{ store: string }> }) {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#f0f2f2" }}>
      ...
      {/* Navbar */}
      <div className="sticky top-0 z-20 border-b border-white/5" style={{ background: "#131921" }}>
          ...
          <div className="flex items-center gap-3 min-w-0">
            {seller.logo_url ? (
              <Image
                ...
                width={38}
                height={38}
                className="w-9 h-9 rounded-lg object-cover flex-shrink-0 ring-1 ring-white/20"
              />
            ) : (
              <div
            ...
            <p className="text-white text-base font-semibold capitalize truncate">{storeName_}</p>
          </div>
          <CartIconButton />
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-6 py-5">
        ...
        {newArrivals.length > 0 && <NewArrivalsCarousel products={newArrivals} />}
        {(bestSellers.length > 0 || newArrivals.length > 0) && (
          <p className="text-sm font-bold text-gray-900 mb-3">All Products</p>
        )}
